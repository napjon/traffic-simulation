...
void Vehicle::simulate()
{
    ...
    threads.emplace(std::thread(&Vehicle::drive, this)):
}
...
std::cout << "Vehicle #" << id << "::drive: thread id" << std::this_thread::get_id() << std::endl;
    ...
    while (true)
    {
        // sleep at every iterations
        std::this_thread::sleep_for(std::chrono::milliseconds(1));

        // compute the difference to stop watch
        long timeSinceLastUpdate = std::chorono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - lastUpdate).count();
        if (timeSinceLastUpdate >= cycleDuration)
        {
            // update position with a  constant velocity motion model
            _posStreet += _speed * timeSinceLastUpdate  /10000;

            // compute completion rate of current street
            double completion = _posStreet / _currStreet->getLength();

            // compute current pixel position on street based on driving duration
            std::shared_ptr<Intersection> i1, i2;

            i2 = _currDestination;
            i1 = i2->getID() == _currStreet->getIntersection()->getID() ? _currStreet->getOutIntersection() : _currStreet->getIntersection();
            ...
            {
                // request entry to the current intersection (using aysnc)
            ...
            if (completion >= 1.0 & hasEnteredIntersection)
            {
                ...
                _currDestination->vehiceleHasLeft(get_shared_this());
