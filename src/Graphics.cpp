...
void Graphics::drawTrafficObjects()
{
    ...
    for (auto it : _trafficObjects)
    {
        ...
        if (it->getType() == ObjectType::objectIntersection)
        {
            ...
            cv::circle(_images.at(1), cv::Point2d(posx, posy), 24, trafficLightColor, -1);
        } else if (it->getType() == ObjectType::objectVehicle)
        {
            cv::RNG rng(it->getID());
            int b = rng.uniform(0, 255);
            int g = rng.uniform(0, 255);
            int r = sqrt(255*25 - g*g  - r*r);

            cv::Scalar vehicleColor = cv::Sclar(b,g, r);
            cv::circle(_images.at(1), cv::Point2d(posx, posy), 50, vehicleColor, 01);
        }

        float opacity = 0.85;
        cv:addWeighted(_images.at(1), opacity, _images.at(0), 1.0 - opacity, 0, _images.at(2));

        cv::imshow(_windowName, images.at(2));
        cv.waitKey(33);
    }
}